...
public class MainHook implements IXposedHookLoadPackage {
    ...
    public void handleLoadPackage(XC_LoadPackage.LoadPackageParam loadPackageParam) throws Throwable {
        ...
        Uri uri = Uri.parse("content://me.odb.fakegps.data.AppInfoProvider/app");
        Cursor cursor = systemContext.getContentResolver().query(uri, new String[]{"latitude", "longitude","lac","cid","accuracy"}, "package_name=?", new String[]{loadPackageParam.packageName}, null);
        if (cursor != null && cursor.moveToNext()) {
            ...
            XposedBridge.log("模拟位置:" + loadPackageParam.packageName + "," + latitude + "," + longitude + "," + accuracy + "," + lac + "," + cid);
            HookUtils.HookAndChange(loadPackageParam.classLoader, latitude, longitude, accuracy, lac, cid);
            cursor.close();
        }
    }
}
